...
class Bowling
    ...
    def add_score(pins)
        ...
        @temp << pins
        #2投分のデータが入っているか、1投目がストライクだったら、1フレームのスコアとして全体に追加する
    ...
    #スコアの合計を計算する
    def calc_score
        @scores.each.with.indes(1) do |score,indes|
          #最終フレーム以外でのストライクなら、スコアにボーナスを含めて合計する
            if strike?(score) && not_lats_frame?(index)
                @total_score += calc_strike_bonus(index)
            # 最終フレーム以外でのスペアなら、スコアにボーナスを含めて合計する
            elsif spare?(score) && not_lats_frame?(index)
    ...
    def calc_strike_bonus(index)
       ...
       if strike?(@scores[index]) && not_lats_frame?(index)
